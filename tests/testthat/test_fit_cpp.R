...
# Tests for the fit_cpp C++ functions
...
test_that("Densities are calculated correctly", {
  ...
  # Lognormal
  # x <- 1:10
  # calculated <- compute_densities(x, 2, "loggaussian")       # F, 1-F, f, f'
  # expected <- cbind(plnorm(x), 1-plnorm(x), dlnorm(x))
  # expect_equal(calculated[, 1], expected[, 1])
  # expect_equal(calculated[, 2], expected[, 2])
  # expect_equal(calculated[, 3], expected[, 3])
  # expect_equal(calculated[, 4], expected[, 4])  # TODO
