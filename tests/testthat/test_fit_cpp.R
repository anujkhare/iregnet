...
# to test the distribution funciton in normal values
eta<-c(863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121, 863.121)

fit <- compute_grad_response_cpp(y_l, y_r,
                          eta, 568.773, ovarian$fustat,
                          "gaussian")

# fit <- iregnet(X, y, family="gaussian", alpha=1, intercept = T, scale = 541.5972)
# fit <- iregnet(X, y, family="gaussian", alpha=1, intercept = T)
# fit <- iregnet(X, y, family="loggaussian", alpha=1, intercept = T, scale = 1.1813)
# fit <- iregnet(X, y, family="loggaussian", alpha=1, intercept = T)
print(fit)
# fit <- iregnet(X, y, family="gaussian", alpha=1, intercept = T, scale = 300)
# #print(fit)
# print(fit)

# TODO: Exponetial troubles!
# fit <- iregnet(X, y, family="exponential", alpha=1, intercept = T)
# print(fit)
print("No censoring:")
...
# TODO: Lambda not calculated correctly - as apparent from this example:
# rm(list=ls())
# load("/home/kcm/code/xy")
# X <- a[[1]]
# y <- a[[2]]
...
# y <- cbind(y, y)
# print (y)
# fit <- iregnet(X, y, family = "gaussian", alpha=1, intercept = T, scale = 0.6558668)
# print (fit)
